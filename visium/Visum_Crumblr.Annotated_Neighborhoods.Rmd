...
```{r}
nhood_clus_anno <- c("DEJ","KC_Stress","Stroma","TLS","Diff_IFE","Seb_gland","Stroma","Perivasc_II","TLS_like","Lower_HF",
                    "Diff-IFE","Seb_gland","Eccrine","BCC_tumor","HS_tunnel","Perivasc_I","Upper_HF","Subcutis","Eccrine","BCC_low")
names(nhood_clus_anno) <- 0:19

```

```{r}
obj@meta.data$neighborhood <- factor(obj@meta.data$harmony.snn_res.0.6, 
                                     levels = names(nhood_clus_anno),
                                     labels=nhood_clus_anno)
```
...
resdf <- lapply(1:length(conditions), function(x){
  ...
  df$neighborhood <- factor(rownames(df), levels = unique(nhood_clus_anno))
  return(df)
}) %>% bind_rows()
...
heatmap <- ggplot(resdf) + 
  aes(x = neighborhood, y = tissue_type, fill = logFC,
...
vpdf <- data.frame(vp)
cols <- colnames(vpdf)
vpdf$neighborhood <- factor(rownames(vpdf), levels = unique(nhood_clus_anno))
...
vplot <- ggplot(vpdf) + 
  aes(x = neighborhood, y=value, fill = variable) + 
...
```{r}
lfc.mat <- resdf %>%
  dplyr::select(tissue_type, neighborhood, logFC) %>%
  pivot_wider(names_from = "neighborhood", values_from = "logFC") %>%
  data.frame()
rownames(lfc.mat) <- lfc.mat$tissue_type
lfc.mat$tissue_type <- NULL
lfc.mat <- lfc.mat[, make.names(levels(obj@meta.data$neighborhood))]
colnames(lfc.mat) <- make.names(levels(obj@meta.data$neighborhood))

p.mat <- resdf %>%
  mutate(padj.star = gsub("\\.", "", gtools::stars.pval(adj.P.Val))) %>%
  dplyr::select(tissue_type, neighborhood, padj.star) %>%
  pivot_wider(names_from = "neighborhood", values_from = "padj.star") %>%
  data.frame()
rownames(p.mat) <- p.mat$tissue_type
p.mat$tissue_type <- NULL
p.mat <- p.mat[, make.names(levels(obj@meta.data$neighborhood))]
colnames(p.mat) <- make.names(levels(obj@meta.data$neighborhood))
```
